...
void deception_filter_find(vmi_instance_t vmi, drakvuf_trap_info *info, drakvuf_t drakvuf, std::string target, std::string& replacement_string)
{
    ApimonReturnHookData *data = (ApimonReturnHookData *)info->trap->data;
    std::vector<uint64_t> temp_args = data->arguments;
    vmi_pid_t curr_pid = info->attached_proc_data.pid;
    ...
    size_t xBuffSize;
    const size_t REPLACEMENT_SIZE = replacement_string.length() * 2;

    unsigned char* charArray = new unsigned char[REPLACEMENT_SIZE];
    for (size_t i = 0; i < replacement_string.length(); i++)
    {
        charArray[i * 2 ] = static_cast<unsigned char>(replacement_string[i]);
        charArray[i * 2 + 1] = 0;
    }
    charArray[REPLACEMENT_SIZE] = '\0'; // Null-terminate the array
    ...
    if (VMI_FAILURE == vmi_read_va(vmi, xBuffLocation, curr_pid, xBuffSize, aFilterBuff, NULL)) // reading and storing buffer
    {
    ...
    std::vector<char> vec(target.begin(), target.end());
    for (size_t i = 0; i < xBuffSize; i++) // looping through buffer and looking for WdFilter
    {
       
        if (aFilterBuff[i] == vec[0] && aFilterBuff[i + 2] == vec[1])
        {
            for (size_t j = 0; j < REPLACEMENT_SIZE; j++)
            {
                aFilterBuff[i + j] = charArray[j];
            }

            if (VMI_FAILURE == vmi_write_va(vmi, xBuffLocation, curr_pid, xBuffSize, aFilterBuff, NULL)) // writing altered buffer
            {
                std::cout << "Failed to read memory from VMI (this is next check) "
                          << &aFilterBuff << "\n";
                return;
