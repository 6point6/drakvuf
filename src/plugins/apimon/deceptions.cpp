...
/*###################### HideFile WIP ################################*/
void deception_find_first_or_next_file_a(vmi_instance_t vmi, drakvuf_trap_info *info, char filename[], uint8_t *fake_filename)
{
    ApimonReturnHookData *data = (ApimonReturnHookData *)info->trap->data; // Get the data from the trap
    std::vector<uint64_t> temp_args = data->arguments;                     // Store all the arguments passed by the function
    vmi_pid_t curr_pid = info->attached_proc_data.pid;                     // Get PID of process
    ...
    char *cFileName = new char[MAX_PATH]; // 260 

    addr_t cFileNameAddr = lpFindFileData + 44; // 5 DWORDS + 3 FILETIMES = 44
    if (VMI_FAILURE == vmi_read_va(vmi, cFileNameAddr, curr_pid, MAX_PATH, cFileName, NULL))
    {
        std::cout << "Failed to read memory from VMI\n";
        return;
    }
    //std::cout << "cFileName: " << cFileName << "\n";
 
    if (strcmp(cFileName, filename) == 0)
    {
        if (VMI_FAILURE == vmi_write_va(vmi, cFileNameAddr, curr_pid, MAX_PATH, fake_filename, NULL))
        {
            std::cout << "Writing to mem failed!" << "\n";
            return;
        }
