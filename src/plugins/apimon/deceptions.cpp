...
void deception_net_user_enum(vmi_instance_t vmi, drakvuf_trap_info* info) {
    ...
    std::cout << "pUserInfo_0: 0x" << std::hex << pUserInfo_3 << "\n";
...
if(username) {
    ...
    if(usernameStr == "HIGHPRIV") {
        ...
        if (VMI_FAILURE == vmi_write_va(vmi, pUsri0_name, curr_pid, nullChar, sizeof(nullChar))) {
        ...
        if(domainandname) {
            ...
            if(pos != std::string::npos) {
                ...
                if(accountName == "HIGHPRIV") {
                    ...
                    std::cout << "HIGHPRIV account nulled!" << "\n";
                    if (VMI_FAILURE == vmi_write_va(vmi, pLgrmi3_domainandname + pos + 1, curr_pid, nullChar, sizeof(nullChar))) { // Write the null character
                        std::cout << "Writing to memory failed!" << "\n";
                    }
                } else {
...
void deception_lookup_account_sidw(vmi_instance_t vmi, drakvuf_trap_info* info) {
    ...
    if (VMI_FAILURE == vmi_read_va(vmi, pSID, curr_pid, current_SID, sidLength)) {
