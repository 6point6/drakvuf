 ...
 */
static inline void process_free_requests(drakvuf_t drakvuf)
{
    GHashTableIter i;
    addr_t *key = NULL;
...
{
    event_response_t rsp = 0;
    addr_t pa = (event->interrupt_event.gfn << 12) + event->interrupt_event.offset + event->interrupt_event.insn_length - 1;
